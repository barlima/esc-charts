...
export default async function SongPage({
  ...
  const { year: contestYear, country: countrySlug } = await params;
  ...
  // Fetch song data directly using supabase
  let song: (Song & { country_id: number }) | null = null;
  let songError: string | null = null;
  ...
  try {
    ...
    if (countryError) {
      songError = countryError.message;
    } else if (countryData) {
      // Then get the song using the country ID
      ...
      if (songDataError) {
        // Try to get semifinal song if final doesn't exist
    ...
    songError = err instanceof Error ? err.message : 'Unknown error';
  }
  ...
  if (song.venue_type === 'final') {
    ...
    if (semifinalData) {
      ...
      const positionResult = await getSongPosition(semifinalData.id, semifinalData.venue_type as VenueType);
      semifinalPosition = positionResult.position;
    }
  }
      ...
      <Flex direction="column" gap="6">
        ...
        <Box>
          <Flex gap="2" align="center">
            ...
            <Text size="2">{song.country_name}</Text>
          </Flex>
        </Box>

        {/* Country name heading */}
        <Heading size="8">{song.country_name}</Heading>
